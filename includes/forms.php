        ...
        if (is_array($api)) {
            ...
            foreach ($brands as $form) {
                if (!caag_hq_rental_exists($form->id)) {
                    ...
                    add_post_meta($post_id, CAAG_HQ_RENTAL_FIRST_STEP_LINK,
                        esc_url_raw($form->public_reservations_link_first_step));
                    if (isset($form->public_packages_link_full) and $form->public_packages_link_full != '') {
                        add_post_meta($post_id, CAAG_HQ_RENTAL_PUBLIC_PACKAGES_LINK,
                            esc_url_raw($form->public_packages_link_full));
                        add_post_meta($post_id, CAAG_HQ_RENTAL_FIRST_STEP_LINK_PACKAGES,
                            esc_url_raw($form->public_packages_link_first_step));
                        add_post_meta($post_id, CAAG_HQ_RENTAL_SHORTCODE_PACKAGES,
                            '[caag_hq_rental_forms_packages id=' . $form->id . ']');
                    }
                    if (isset($form->public_reservations_packages_link_first_step) and
                        $form->public_reservations_packages_link_first_step != '') {
                        add_post_meta($post_id, CAAG_HQ_RENTAL_PUBLIC_RESERVATION_PACKAGES_LINK,
                            esc_url_raw($form->public_reservations_packages_link_first_step));
                        add_post_meta($post_id, CAAG_HQ_RENTAL_SHORTCODE_RESERVATION_PACKAGES,
                            '[caag_hq_rental_forms_reservation_packages id=' . $form->id . ']');
                    }
                } else {
                    $post = get_caag_hq_rental_by_meta($form->id)[0];
                    update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_LINK,
                        esc_url_raw($form->public_reservations_link_full));
                    update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_FIRST_STEP_LINK,
                        esc_url_raw($form->public_reservations_link_first_step));
                    update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_SHORTCODE,
                        '[caag_hq_rental_forms id=' . $form->id . ']');
                    if (isset($form->public_packages_link_full) and $form->public_packages_link_full != '') {
                        update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_PUBLIC_PACKAGES_LINK,
                            esc_url_raw($form->public_packages_link_full));
                        update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_FIRST_STEP_LINK_PACKAGES,
                            esc_url_raw($form->public_packages_link_first_step));
                        update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_SHORTCODE_PACKAGES,
                            '[caag_hq_rental_forms_packages id=' . $form->id . ']');
                    }
                    if (isset($form->public_reservations_packages_link_first_step) and
                        $form->public_reservations_packages_link_first_step != '') {
                        update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_PUBLIC_RESERVATION_PACKAGES_LINK,
                            esc_url_raw($form->public_reservations_packages_link_first_step));
                        update_post_meta((int)$post->post_id, CAAG_HQ_RENTAL_SHORTCODE_RESERVATION_PACKAGES,
                            '[caag_hq_rental_forms_reservation_packages id=' . $form->id . ']');
                    }
                    $args = array(
                        'ID' => (int)$post->post_id,
